...
- Поддержка автоматического применения _at.cfg из всех зависимостей проекта при setupDecompWorkspace. По умолчанию отключена, для активации указать useAtFromDependencies=true в minecraft конфигурации
...
- Добавлен параметр excludeTwitchLib (по умолчанию true) для исключения твич библиотеки из Minecraft
...
minecraft {
    ...
    //Если нужно включить AT из модов - раскомментируйте эту строку
    //useAtFromDependencies=true
    
    // Если нужно включить Scala библиотеки (по умолчанию они исключаются)
    //excludeScalaLibs=false

    // Если нужно включить twitch библиотеку (по умолчанию она исключается)
    //excludeTwitchLib=false
}
```
